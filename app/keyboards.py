...
def service_keyboard_from_services(services, page: int, max_page: int, order_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    for service in services:
        ...
        kb.button(text=f"🧩 › {format_service_label(service)}", callback_data=f"service:{order_id or 0}:{slug}")
    if page > 0:
...
def service_confirm_keyboard(order_id: int, service_slug: str) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ › Подтвердить сервис", callback_data=f"service_confirm:{order_id}:{service_slug}")
    ...
    if active_rows:
        for admin in active_rows:
            ...
            if admin.telegram_id in env_admin_ids:
                ...
                kb.button(text=f"🗑 › {admin.telegram_id} — {name}", callback_data=f"admin:remove_admin:{admin.telegram_id}")
    else:
...
def admin_order_card_keyboard(order_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="🔁 › Повторить запрос", callback_data=f"admin:order_resend:{order_id}")
...
def supplier_section_orders_keyboard(rows, mode: str = "active", page: int = 0, max_page: int = 0) -> InlineKeyboardMarkup:
    ...
    for request, order in rows:
        ...
        if status == "waiting_buyer_confirm":
            kb.button(text=f"⏳ › #{op_id} — {title} — ждём OK", callback_data=f"supplier:wait:{request.id}:{mode}:{page}")
            continue
        icon = "📞" if request.request_type == "number" else "🔑"
        action = "номер" if request.request_type == "number" else "код"
        kb.button(text=f"{icon} › #{op_id} — {title} — {action}", callback_data=f"supplier:reqf:{request.id}:{mode}:{page}")
    if page > 0:
        kb.button(text="⬅️ › Назад", callback_data=f"supplier:filter:{mode}:{page - 1}")
    if page < max_page:
        kb.button(text="➡️ › Дальше", callback_data=f"supplier:filter:{mode}:{page + 1}")
...
def buyer_main_reply_keyboard(is_admin: bool = False) -> ReplyKeyboardMarkup:
    """Постоянная панель покупателя под полем ввода — без команд со слешем."""
    rows = [
    ...
    if is_admin:
        rows.extend([
            ...
            [KeyboardButton(text="💳 Оплата"), KeyboardButton(text="📢 Рассылка")],
...
def admin_category_select_keyboard(categories) -> InlineKeyboardMarkup:
    ...
    for row in categories:
        kb.button(text=f"{row.emoji} {row.name}", callback_data=f"admin:shop:wizard_category:{row.id}")
...
def confirm_delete_product_keyboard(product_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ Удалить", callback_data=f"admin:shop:product_delete_confirm:{product_id}", style="danger")
    kb.button(text="⬅️ Отмена", callback_data=f"admin:shop:product:{product_id}", style="danger")
    kb.adjust(1)
    return kb.as_markup()
...
def confirm_delete_category_keyboard(category_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ Удалить", callback_data=f"admin:shop:category_delete_confirm:{category_id}", style="danger")
    kb.button(text="⬅️ Отмена", callback_data=f"admin:shop:category:{category_id}", style="danger")
    kb.adjust(1)
    return kb.as_markup()


def supplier_selected_request_keyboard(request_id: int, request_type: str) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(
        text="✍️ Отправить номер" if request_type == "number" else "✍️ Отправить код",
        callback_data=f"supplier:answer:{request_id}",
    )
    kb.button(text="❌ Отменить выбор", callback_data=f"supplier:cancel_selection:{request_id}", style="danger")
