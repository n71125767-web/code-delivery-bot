...
def get_business_id_from_message(message: Message) -> str | None:
    return getattr(message, "business_connection_id", None) or ADMIN_BUSINESS_CONNECTION_ID


async def notify_admins(bot: Bot, text: str) -> None:
    # Уведомления админу лучше слать обычным ботом, иначе Business-аккаунт может писать сам себе.
...
async def process_admaker_message(bot: Bot, message: Message) -> None:
    ...
    if not data:
        await notify_admins(
            bot,
            "Shop-бот прислал сообщение, но я не смог распарсить покупку.\n\n"
            f"Текст:\n{text}",
        )
        return
...
async def handle_buyer_message(bot: Bot, message: Message, business_connection_id: str | None) -> None:
    ...
    async with SessionLocal() as session:
        ...
        if not order:
            await answer_message(
                ...
                message,
                "Заказ не найден.\n\n"
                "Если вы уже оплатили, напишите админу или нажмите /start в боте после оплаты.",
                business_connection_id,
            )
            await notify_admins(
                bot,
    ...
    # Поставщику отправляем через тот же Business ID, если поставщик пишет/доступен этому Business.
    # Если Telegram не даст — fallback обычным ботом.
    supplier_ok = await safe_send_message(bot, supplier_id, supplier_text, business_connection_id)
    if not supplier_ok:
        supplier_ok = await safe_send_message(bot, supplier_id, supplier_text)

    if not supplier_ok:
        await answer_message(bot, message, "Сервис принят, но я не смог написать поставщику. Админ уже уведомлён.", business_connection_id)
        await notify_admins(bot, f"Не смог отправить поставщику заявку по заказу #{order.operation_id}")
        return
...
async def route_text_message(bot: Bot, message: Message, is_business: bool) -> None:
    if not message.from_user:
        return
    ...
    text = (message.text or "").strip()
    business_connection_id = get_business_id_from_message(message) if is_business else None

    logger.info(
        "ROUTE_TEXT is_business=%s from_id=%s username=%s is_bot=%s chat_id=%s business_id=%s text=%s",
        is_business,
        user_id,
        username,
        sender.is_bot,
    ...
    # Защита от цикла: не обрабатываем сообщения самого бота.
    if user_id == me.id:
        logger.info("Ignored own bot message")
        return

    # Защита от цикла: админский аккаунт/свои исходящие сообщения не считаем покупателем.
    if is_admin(user_id) and not text.startswith("/"):
        logger.info("Ignored admin non-command message to avoid self-cycle")
        return

    # Не обрабатываем чужих ботов, кроме shop-бота.
...
async def business_text_router(message: Message, bot: Bot) -> None:
    await route_text_message(bot, message, is_business=True)


@router.message(F.text)
async def normal_text_router(message: Message, bot: Bot) -> None:
    await route_text_message(bot, message, is_business=False)


@router.callback_query(F.data.startswith("code_sent:"))
async def code_sent_callback(callback: CallbackQuery, bot: Bot) -> None:
...
async def confirm_success_callback(callback: CallbackQuery) -> None:
    order_id = int(callback.data.split(":")[1])
...
async def number_invalid_callback(callback: CallbackQuery, bot: Bot) -> None:
    order_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        order = await get_order_by_id(session, order_id)
        if not order:
...
async def code_invalid_callback(callback: CallbackQuery, bot: Bot) -> None:
    order_id = int(callback.data.split(":")[1])
    async with SessionLocal() as session:
        order = await get_order_by_id(session, order_id)
        if not order:
...
@router.update()
async def debug_unhandled_update(update: Update, bot: Bot) -> None:
    """
    Если снова увидишь в Render 'Update is not handled',
    этот обработчик должен начать печатать тип апдейта.
    """
    update_type = None
    for name in [
        "message",
    ...
    logger.info("DEBUG CATCH-ALL UPDATE type=%s update_id=%s", update_type, update.update_id)
