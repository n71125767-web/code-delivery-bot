...
async def send_supplier_request_for_order(
    ...
    # Поставщику НЕ уходит текст покупателя, фото, username или контакт.
    supplier_text = (
        "Новый заказ.\n\n"
        f"Заказ: #{order.operation_id}\n"
        f"ID в базе: {order.id}\n"
...
async def accept_service_for_order(
    ...
    async with SessionLocal() as session:
        ...
        # ВАЖНО: не затираем старый Business ID значением None.
    ...
    ok = await send_supplier_request_for_order(
        bot,
        order,
        actual_business_id,
    ...
    if message:
        if ok:
            await answer_message(bot, message, "OK. Сервис принят. Ожидайте номер.", actual_business_id)
        else:
            await answer_message(
                bot,
                message,
...
async def handle_buyer_message(bot: Bot, message: Message, business_connection_id: str | None) -> None:
    ...
    async with SessionLocal() as session:
        ...
        # ВАЖНО: сохраняем Business ID только если он реально есть.
        if business_connection_id:
            order.business_connection_id = business_connection_id
    ...
    async with SessionLocal() as session:
        ...
        if number_request:
            ...
            if not ok:
                ...
                await notify_admins(
                    bot,
                    "Не смог отправить номер покупателю.\n\n"
                    f"Заказ #{order.operation_id}\n"
                    f"Покупатель chat_id: {target_chat_id}\n"
                    f"business_connection_id: {target_business_id}",
                )
                return
        ...
        if code_request:
            ...
            if not ok:
                ...
                await notify_admins(
                    bot,
                    "Не смог отправить код покупателю.\n\n"
                    f"Заказ #{order.operation_id}\n"
                    f"Покупатель chat_id: {target_chat_id}\n"
                    f"business_connection_id: {target_business_id}",
                )
...
async def route_message(bot: Bot, message: Message, is_business: bool) -> None:
    ...
    # Поставщик определяется по базе suppliers.
    async with SessionLocal() as session:
...
async def handle_callback(bot: Bot, callback: CallbackQuery) -> None:
    ...
    if data.startswith("service:"):
        ...
        if order_id:
            ...
            await accept_service_for_order(bot, message, order_id, service_name, business_id)
