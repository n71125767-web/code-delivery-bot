...
def extract_purchase_data(text: str) -> dict | None:
    if not text:
        return None
    operation = _search(
        r"(?:ID\s*операции|Заказ|Operation\s*ID)\s*[:#№-]*\s*(\d+)", text
    )
    status = _search(r"Статус\s*:\s*([A-ZА-Я_]+)", text)
    paid_marker = bool(
        re.search(r"(?:Статус\s*:\s*PAID|Новая\s+покупка|Оплачен\s*:)", text, re.I)
    )
    if not operation or not paid_marker:
        return None
    product_id = _search(r"ID\s*товара\s*:\s*(\d+)", text)
    user_id = _search(r"ID\s*пользователя\s*:\s*(\d+)", text) or _search(
        r"(?<!Внешний\s)ID\s*:\s*(\d+)", text
    )
...
def extract_phone(text: str) -> str | None:
    ...
    for item in candidates:
        normalized = ("+" if item.strip().startswith("+") else "") + re.sub(
            r"\D", "", item
        )
